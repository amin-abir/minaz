...
if ( ! function_exists( 'minaz_posted_by' ) ) :
	...
	function minaz_posted_by() {
		$byline = sprintf(
			...
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' .esc_url(get_avatar()). esc_html( get_the_author() ) . '</a></span>'
		);
	...
	function minaz_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'minaz' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
